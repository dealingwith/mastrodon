...
export async function loadMastodonArchive(): Promise<MastodonPost[]> {
  try {
    // Try to load from public/mastodon-archive.json
    const response = await fetch('/mastodon-archive.json');
    
    if (!response.ok) {
      console.log('No mastodon-archive.json found in public folder');
      return [];
    }
    ...
    if (data.orderedItems) {
      // ActivityPub format
      posts = data.orderedItems.filter((item: any) => 
...
function transformToMastodonPost(item: any): MastodonPost {
  return {
    ...
    media_attachments: item.attachment?.map((att: any) => ({
    ...
    account: {
      ...
      display_name: item.actor?.name || extractUsername(item.attributedTo) || 'User',
